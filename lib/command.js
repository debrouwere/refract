...
(function() {
  var additionalHelpers, context, fs, helpers, indentation, inputLocation, inputPath, normalize, normalizeString, normalizedRefraction, object, program, rawInput, rawTemplate, refract, refractTemplate, refraction, serialization, template, templatePath, yaml, _, _ref;
  ...
  program.option('-a --add', 'Add refracted fields to the original object.').option('-d --defaults', 'Only refract a field if it is not present in the original object.').option('-e --each [key]', 'Refract each element in an array.').option('-H --helpers', 'Add in additional JavaScript helper functions.').option('-i --in-place', 'Modify the file that contains the original object.').option('-n --normalize [style]', 'Normalize field names to a standard style.').option('-p --pretty', 'Output pretty indented JSON.').parse(process.argv);
