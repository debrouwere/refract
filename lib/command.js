...
(function() {
  var additionalHelpers, fs, helpers, inputPath, load, merge, normalize, normalizedObjects, normalizer, objects, pairs, program, refract, refractions, serialization, template, type, utils, _,
    __slice = [].slice;
  ...
  additionalHelpers = (program.helpers || '').split(',').map(function(path) {
    return load(path, {
      namespace: true
    });
  });
