...
(function() {
  var additionalHelpers, fs, helpers, inputLocation, inputPath, merge, normalize, normalizedObjects, normalizer, objects, pairs, program, rawInput, rawTemplate, refract, refractions, serialization, template, utils, yaml, _;
  ...
  program.option('-t --template <path>', 'Path to a template file.').option('-s --string <template>', 'A template string.').option('-a --apply <mapping>', 'A mapping that specifies which function to apply to which field.').option('-u --update', 'Add refracted fields to the original object.').option('-m --missing', 'Only refract a field if it is not present in the original object.').option('-e --each', 'Refract each element in an array.').option('-H --helpers <path>', 'Add in additional JavaScript helper functions.').option('-i --in-place', 'Modify the file that contains the original object.').option('-n --normalized <style>', 'Normalize field names to a standard style.', 'underscored').option('-I --indent [n]', 'Output pretty indented JSON.', parseInt, 2).parse(process.argv);
