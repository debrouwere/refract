...
(function() {
  var additionalHelpers, fs, helpers, inputLocation, inputPath, merge, normalize, normalizedObjects, normalizer, objects, pairs, parse, program, rawInput, rawTemplate, refract, refractions, serialization, template, utils, yaml, _;
  ...
  if (program["new"]) {
    ...
    if ((fs.path.extname(inputLocation)) === '.json') {
      parse = JSON.parse;
    } else {
      parse = yaml.safeLoad;
    }
