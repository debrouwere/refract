...
(function() {
  var additionalHelpers, context, fs, helpers, indentation, inputLocation, inputPath, normalize, normalizeString, normalizedRefraction, object, program, rawInput, rawTemplate, refract, refractTemplate, refraction, serialization, template, yaml, _;
  ...
  program.option('-t --template <path>', 'Path to a template file.').option('-s --string <template>', 'A template string.').option('-a --apply <mapping>', 'A mapping that specifies which function to apply to which field.').option('-u --update', 'Add refracted fields to the original object.').option('-m --missing', 'Only refract a field if it is not present in the original object.').option('-e --each', 'Refract each element in an array.').option('-H --helpers', 'Add in additional JavaScript helper functions.').option('-i --in-place', 'Modify the file that contains the original object.').option('-n --normalize <style>', 'Normalize field names to a standard style.').option('-p --pretty', 'Output pretty indented JSON.').parse(process.argv);
  ...
  normalizeString = _.str[program.normalize || 'identity'];

  normalize = function(obj) {
    if (obj.constructor === Object) {
      return _.object(_.map(obj, function(value, key) {
        return [normalizeString(key), normalize(value)];
      }));
    } else {
      return obj;
    }
  ...
  serialization = JSON.stringify(normalizedRefraction, void 0, indentation);
