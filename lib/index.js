...
(function() {
  ...
  extractKeys = function(key) {
    ...
    if (matches > 5) {
      rawKey = match[0], key = match[1], __ = match[2], namespace = match[3], __ = match[4], source = match[5];
    } else {
      rawKey = match[0], key = match[1], __ = match[2], source = match[3];
      if (source == null) {
        source = key;
      }
      namespace = false;
      ...
      case Object:
        return _.object(_.map(template, function(value, key) {
          ...
          if ((iterationOptions = key.slice(-1)) === ']') {
            _ref = extractKeys(key), rawKey = _ref.rawKey, key = _ref.key, namespace = _ref.namespace, source = _ref.source;
